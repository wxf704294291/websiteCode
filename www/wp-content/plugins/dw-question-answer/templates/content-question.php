...
<div class="<?php echo dwqa_post_class(); ?>">
	...
	<div class="dwqa-question-meta">
		...
		?>
		<?php printf( __( '<span><a href="%s">%s%s</a> %s %s 以前</span>', 'dwqa' ), dwqa_get_author_link( $user_id ), get_avatar( $user_id, 48 ), get_the_author(), $text, $time ) ?>
	...
	<div class="dwqa-question-stats">
		<span class="dwqa-views-count">
			...
			<?php printf( __( '<strong>%1$s</strong> 浏览数', 'dwqa' ), $views_count ); ?>
		</span>
		<span class="dwqa-answers-count">
			<?php $answers_count = dwqa_question_answers_count(); ?>
			<?php printf( __( '<strong>%1$s</strong> 回答数', 'dwqa' ), $answers_count ); ?>
		</span>
		<span class="dwqa-votes-count">
			<?php $vote_count = dwqa_vote_count() ?>
			<?php printf( __( '<strong>%1$s</strong> 投票数', 'dwqa' ), $vote_count ); ?>
		</span>
	</div>
</div>
