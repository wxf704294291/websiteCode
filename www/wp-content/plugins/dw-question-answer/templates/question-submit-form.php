...
<?php if ( dwqa_current_user_can( 'post_question' ) ) : ?>
	...
	<form method="post" class="dwqa-content-edit-form" enctype="multipart/form-data">
		<p class="dwqa-search">
			<label for="question_title"><?php _e( '标题', 'dwqa' ) ?></label>
		...
		<p>
			<label for="question-tag"><?php _e( '标签', 'dwqa' ) ?></label>
		...
		<p>
			<label for="_dwqa_anonymous_email"><?php _e( '邮件地址', 'dwqa' ) ?></label>
		...
		<p>
			<label for="_dwqa_anonymous_name"><?php _e( '您的姓名', 'dwqa' ) ?></label>
		...
		<input type="submit" name="dwqa-question-submit" value="<?php _e( '提交', 'dwqa' ) ?>" >
	</form>
